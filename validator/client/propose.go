...
func (v *validator) ProposeBlock(ctx context.Context, slot primitives.Slot, pubKey [fieldparams.BLSPubkeyLength]byte) {
	...
	if err := logProposedBlock(blk, blkResp.BlockRoot); err != nil {
...
// logProposedBlock logs the proposed block.
func logProposedBlock(blk interfaces.SignedBeaconBlock, blkRoot []byte) error {
	...
	log.WithFields(logrus.Fields{
		"slot":             blk.Block().Slot(),
		"blockRoot":        fmt.Sprintf("%#x", bytesutil.Trunc(blkRoot)),
